...
- **WhatsApp group — created (2026-07-16), initial volunteers added.** Serves (1) finding backup coverage when a volunteer's schedule changes, and (2) organizing occasional in-person get-togethers to build community among volunteers. Set up manually by Lauren (WhatsApp personal isn't available through Rube/Composio, see [[reference_rube]]). The site's FAQ copy ("volunteer WhatsApp chat," confirmation email) already assumed this existed, so no copy changes needed now that it's real.
...
3. ~~Create the volunteer WhatsApp group~~ — done 2026-07-16, group created with initial volunteers added. Open question still: whether to link the sign-up sheet from within it or keep it to 1:1 follow-ups only.
...
- 2026-07-16: WhatsApp volunteer group created, initial volunteers added — the last manual next-step from the 7/15 close-out is now done. Nothing in the site's copy needed to change since it already referenced this group as if live.

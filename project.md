...
## Apps Script code (Version 9, drafted 2026-07-14 — not yet deployed, see below)
```javascript
function doPost(e) {
...
— Friends of Parque Niños Unidos`;
    try {
      MailApp.sendEmail(data.email, subject, body, { from: 'friendsofsfparks@gmail.com', name: 'Friends of Parque Niños Unidos' });
    } catch (err) {
...
```
Note: editing this code alone doesn't update the live `/exec` URL — must deploy a new version via Manage Deployments → pencil icon → Version: New version → Deploy. Sends a bilingual (EN + ES) confirmation email via `MailApp.sendEmail` right after the sheet writes succeed. **Version 8 (2026-07-13):** Version 7's `doPost` failed outright in production — confirmed via the new `Errors` tab (added in v8) that the real cause was a missing `https://www.googleapis.com/auth/script.send_mail` authorization (a plain "Deploy new version" never triggers Google's consent prompt for a newly-added scope; fixed by manually running a throwaway function once to trigger it). Also wrapped the email send in try/catch so a future email failure can never block the sheet writes / roster entry again. **Version 9 (2026-07-14):** Lauren wanted the email to actually originate from friendsofsfparks@gmail.com rather than her personal Gmail. Added `{ from: 'friendsofsfparks@gmail.com', name: 'Friends of Parque Niños Unidos' }` to the `MailApp.sendEmail` call — this only works because friendsofsfparks@gmail.com must first be added and verified as a "Send mail as" alias under Lauren's personal Gmail account (Settings → Accounts and Import → Send mail as); without that verification the `from` option is silently ignored and mail reverts to sending as her personal address.
